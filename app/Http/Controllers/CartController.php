...
class CartController extends Controller
{
    ...
    public function checkout(Request $request)
    {
        ...
        // Update the quantity of each cart item in the database
        foreach ($cartItems as $item) {
            $itemId = $item->id;
            $quantity = $quantityMap[$itemId] ?? 1;

            // Update the quantity of the cart item
            $item->update(['property_quantity' => $quantity]);
            $item->property_quantity = $quantity; // Set the quantity for the view
        }

        // Fetch the updated cart items with property relationships
        $cartItems = Cart::with('property.category')->whereIn('id', $itemIds)->get();
    ...
    public function store(Request $request)
    {
        try {
            ...
            $property_quantity = $property->property_quantity;
            ...
            if ($existingItem) {
                return redirect()->route('cart.index')->with('success', 'Property is already in the cart.');
            }
            ...
            if ($property_quantity > 0) {
                $cartItem->property_quantity = 1;
            } else {
                return redirect()->back()->with('error', 'Property is out of stock.');
            }
            $cartItem->save();
